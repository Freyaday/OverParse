...
namespace OverParse
{
    public partial class MainWindow : Window
    {
            ...
            if(encounterlog.running)
            {
                ...
                foreach(Combatant c in encounterlog.combatants)
                    ...
                    { 
                        ...
                        if(c.PercentDPS>Combatant.maxShare) Combatant.maxShare=c.PercentDPS;
                    }
        ...
        private void CombatantDataItem_MouseEnter(object sender, MouseEventArgs e)
        {
            ((Combatant)((ListViewItem)sender).DataContext).bar.Fill=new SolidColorBrush(Color.FromRgb(255, 255, 255));
            ((ListViewItem)sender).Foreground=((Combatant)((ListViewItem)sender).DataContext).BarBrush;
        }
        private void CombatantDataItem_MouseLeave(object sender, MouseEventArgs e)
        {
            Combatant c = ((Combatant)((ListViewItem)sender).DataContext);
            ((ListViewItem)sender).Foreground=c.bar.Fill;//this is currently White. Doing this to save on object creation.
            c.bar.Fill=c.BarBrush;
        }
    }
